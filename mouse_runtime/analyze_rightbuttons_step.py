...
def read_step(path):
    ...
    if s != int(IFSelect_RetDone): raise RuntimeError((path,s))
...
def explore(shape, typ):
    ...
    while ex.More():
        cur=ex.Current()
        if typ==TopAbs_EDGE: cur=TopoDS.Edge_s(cur)
        elif typ==TopAbs_FACE: cur=TopoDS.Face_s(cur)
        elif typ==TopAbs_SOLID: cur=TopoDS.Solid_s(cur)
        out.append(cur); ex.Next()
    return out
...
def sample_edge(edge,n=81):
    ...
    if not (math.isfinite(a) and math.isfinite(b)) or abs(b-a)>1e8: return []
    pts=[]
    for i in range(n):
        u=a+(b-a)*i/(n-1)
...
for ei,e in enumerate(unique):
    pts=sample_edge(e)
    edge_rows.append({'edge':ei+1,'type':curve_type(e),'length':edge_length(e),'faces':edge_faces[ei],
        'bbox':bbox_pts(pts),'z0_flat':bool(pts and max(abs(p[2]-Z0) for p in pts)<2e-6),
        'endpoints':[pts[0],pts[-1]] if pts else None,'samples':pts})

focus={}
for fi in [111,116,128,129]:
    if 1<=fi<=len(faces):
        focus[str(fi)]={'surface_type':surf_type(faces[fi-1]),
            'edges':[{k:v for k,v in edge_rows[i].items() if k!='samples'} for i in face_edges[fi-1]],
...
def nearest_stats(A,B):
    ...
    ds=[min(dist2(a,b) for b in B) for a in A]
...
for ti in target_ids:
    ...
    for cj,row in enumerate(edge_rows):
        ...
        lr=row['length']/edge_rows[ti]['length'] if edge_rows[ti]['length'] else 999
        score=s1['rms_about_mean']+s2['rms_about_mean']+0.25*(s1['spread']+s2['spread'])
        if min(s1['mean'],s2['mean'])<5.0 and 0.3<lr<3.5:
            matches.append({'target_edge':ti+1,'candidate_edge':cj+1,'target_faces':edge_rows[ti]['faces'],
                'candidate_faces':row['faces'],'target_length':edge_rows[ti]['length'],'candidate_length':row['length'],
                'candidate_type':row['type'],'candidate_z0_flat':row['z0_flat'],'A_to_B':s1,'B_to_A':s2,
                'score':score,'candidate_endpoints':row['endpoints']})
matches.sort(key=lambda x:x['score'])

interesting=[]; focus_face_set={111,116,128,129}
for row in edge_rows:
    if row['type'] not in (0,None) or focus_face_set.intersection(row['faces']):
        interesting.append({k:v for k,v in row.items() if k!='samples'})

out={'reference_sha_expected':'fcd48ca5636fab4cb3f0ed3c69e3f7dbe00cca3c27cbb77c41fdd25bed5238cb',
    ...
    'best_offset_candidates':matches[:100],'interesting_edges':interesting}
OUT.write_text(json.dumps(out,indent=2))
print(json.dumps({'face_count':len(faces),'unique_edges':len(unique),'target_rail_edges':[i+1 for i in target_ids],
    'focus':focus,'best_offset_candidates':matches[:20]},indent=2))

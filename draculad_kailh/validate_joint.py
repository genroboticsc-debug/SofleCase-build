...
def select_right_solder(reference):
    ...
    for i, solid in enumerate(reference.solids()):
        ...
        if 1.0 < solid.volume < 3.0 and b.min.X > 4.0:
            candidates.append((i, solid))
    if len(candidates) != 1:
        raise RuntimeError(f"unable to isolate right solder: {[(i,s.volume,bbox_values(s)) for i,s in candidates]}")
    return candidates[0]


def exact_cut_volume(a, b) -> tuple[float, dict]:
    info = {"success": False, "error": None, "valid": None, "solid_count": None}
    try:
        result = a.cut(b)
        info["valid"] = result.is_valid
        info["solid_count"] = len(result.solids())
        value = total_volume(result)
        info["success"] = math.isfinite(value)
        return value, info
    ...
    for x in xs:
        ...
        try:
            ...
            row.update({
                ...
                "edge_length": sum(e.length for e in edges),
            })
        except Exception as exc:
...
def mesh_boolean_diagnostic(ref_solid, gen_solid, out: Path) -> dict:
    data = {"success": False, "official": False, "method": "manifold mesh diagnostic only"}
    try:
        import trimesh
        export_stl(ref_solid, out / "reference_joint.stl", tolerance=0.005, angular_tolerance=0.02)
        export_stl(gen_solid, out / "generated_joint.stl", tolerance=0.005, angular_tolerance=0.02)
        ...
        data.update({
            ...
            "b_minus_a": bv,
            "xor_volume": av + bv,
            "xor_percent_reference_mesh": 100.0 * (av + bv) / abs(float(r.volume)),
        })
    except Exception as exc:
...
def main() -> None:
    ...
    xor_percent = 100.0 * xor_volume / ref.volume if boolean_success else math.inf
    ...
    report = {
        ...
        "reference": rm,
        "generated": gm,
        "volume_error_percent": 100.0 * abs(gm["volume"] - rm["volume"]) / abs(rm["volume"]),
        "area_error_percent": 100.0 * abs(gm["area"] - rm["area"]) / abs(rm["area"]),
        ...
        "reference_sections": section_diagnostics(ref, [6.60, 6.6351914878, 6.70, 6.80, 6.90, 7.00, 7.01, 7.05, 7.08]),
        "generated_sections": section_diagnostics(gen, [6.60, 6.6351914878, 6.70, 6.80, 6.90, 7.00, 7.01, 7.05, 7.08]),
        "mesh_diagnostic": mesh_boolean_diagnostic(ref, gen, out),
        "target_percent": 0.01,
        "pass": boolean_success and xor_percent < 0.01,
    }
    (out / "joint_validation.json").write_text(json.dumps(report, indent=2, allow_nan=True))

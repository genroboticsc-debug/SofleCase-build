"""Refine the exact bottom-left boss on the positive radius/Z side.

The prior one-sided sweep showed the objective still decreasing as both
parameters approached their recovered nominal values. This extension tests the
previously unsampled +0 to +2 micrometre neighborhood while preserving the
shared boss/bore center and all other recovered features.
"""
...
LINEAR = 0.006722
ANGULAR = 0.270
RADIUS_DELTAS = (0.0, 0.00025, 0.00050, 0.00075, 0.00100, 0.00150, 0.00200)
Z_DELTAS = (0.0, 0.00025, 0.00050, 0.00075, 0.00100, 0.00150, 0.00200)
ROOT = validator.ROOT / "generated" / "bottom_left_boss_positive_sweep"
...
for radius_delta in RADIUS_DELTAS:
    for z_delta in Z_DELTAS:
        ...
        print(
            f"=== positive bottom-left candidate radius_delta={radius_delta:+.7f} "
            ...
            row.update(
                {
                    ...
                    "bottom_left_bore_center_z_mm": bore_z + z_delta,
                }
            )
...
print("=== POSITIVE BOTTOM-LEFT BOSS SWEEP RESULT ===", flush=True)

"""Sweep only the bottom-left outer boss X center, keeping its bore fixed.

The shared boss/bore X sweep had a nominal minimum, but residual localization
shows the dominant error on the outer boss surface. This isolates that outer
cylinder from the independently recovered mounting-bore center.
"""
...
ANGULAR = 0.270
X_DELTAS = (
    -0.00300, -0.00250, -0.00200, -0.00150, -0.00125, -0.00100,
    -0.00075, -0.00050, -0.00025, 0.00000, 0.00025, 0.00050,
     0.00075, 0.00100, 0.00125, 0.00150, 0.00200, 0.00250,
     0.00300, 0.00400, 0.00500,
)
ROOT = validator.ROOT / "generated" / "bottom_left_boss_only_x_sweep"
...
def install_candidate(x_delta: float) -> None:
    tp.BOSSES = (
        (base_name, base_x + x_delta, base_z, base_y0, base_y1),
        *original_bosses[1:],
    )
...
for x_delta in X_DELTAS:
    ...
    print(f"=== bottom-left outer boss-only X delta {x_delta:+.7f} mm ===", flush=True)
    try:
        install_candidate(x_delta)
        row = validator.validate_candidate(LINEAR, reference_raw, reference)
        row.update(
            {
                "bottom_left_boss_center_x_delta_mm": x_delta,
                "bottom_left_boss_center_x_mm": base_x + x_delta,
                "bottom_left_bore_center_x_mm": fixed_bore[1],
                "bottom_left_boss_center_z_mm": base_z,
...
print("=== BOTTOM-LEFT OUTER BOSS-ONLY X SWEEP RESULT ===", flush=True)

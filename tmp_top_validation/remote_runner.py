...
def write_environment() -> None:
    info = {
        ...
        "arial_font_files": command_output(["bash", "-lc", "fc-list | grep -i '/Arial' | head -20"]),
    }
    (GENERATED / "environment.json").write_text(json.dumps(info, indent=2), encoding="utf-8")


def run() -> int:
    write_environment()
    status = {"generation": False, "validation": False, "overall_pass": False}
    try:
        ...
        report = validate(REFERENCE, GENERATED)
        status["validation"] = True
        status["overall_pass"] = bool(report.get("overall_pass"))
    except Exception as exc:
        status["exception"] = f"{type(exc).__name__}: {exc}"
        status["traceback"] = traceback.format_exc()

    (GENERATED / "run_status.json").write_text(json.dumps(status, indent=2), encoding="utf-8")
    ...
    with zipfile.ZipFile(archive, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        for path in (ROOT / "top_parametric.py", ROOT / "validate_top.py", REFERENCE):
            if path.exists():

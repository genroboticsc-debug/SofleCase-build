...
import com.nekospeak.tts.ui.screens.StudioScreen
...
fun MainScreen() {
    ...
    val startDestination = if (prefs.isOnboardingComplete) Screen.Studio.route else Screen.Onboarding.route
    val navBackStackEntry by navController.currentBackStackEntryAsState()
    val currentRoute = navBackStackEntry?.destination?.route
    val hideBottomBarRoutes = listOf(Screen.Onboarding.route, Screen.VoiceRecorder.route, Screen.ModelManager.route)
    ...
    Scaffold(
        ...
        NavHost(
            ...
            composable(Screen.VoiceRecorder.route) {
                VoiceRecorderScreen(
                    navController = navController,
                    onVoiceRecorded = { path, name, transcript -> pendingVoiceCloneData = Triple(path, name, transcript) }
                )
            }
            composable(Screen.ModelManager.route) { ModelManagerScreen(navController = navController) }
        }
    }
}
...
fun BottomNavBar(navController: NavHostController) {
    NavigationBar {
        ...
        val items = listOf(
            ...
            Screen.Studio to Icons.Default.PlayArrow,
